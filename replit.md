...
**Latest additions:**
- **Top-3 Gap Bridge Package (2026-06-09)** — Enterprise zero-trust gap bridge fully applied from ChatGPT audit package (`PROXHQVPN_TOP3_GAP_BRIDGE_PACKAGE`):
  - **RBAC** (`lib/rbac.ts`) — 6-role model: `owner / security_admin / network_admin / auditor / support / user`. 10 fine-grained actions (`vpn:read`, `vpn:write`, `peer:*`, `audit:*`, `admin:write`, `billing:read`, `incident:write`, `ztna:posture`). `can()`, `requirePermission()`, `parseRole()` helpers.
  - **Device Trust / ZTNA** (`lib/device-trust.ts`) — Posture scoring engine with 8 signal dimensions (disk encryption, firewall, EDR, root/jailbreak, patch age, certificate, IP reputation). Score ≥ 75 = allow; below = deny with reasons + remediation. `signalsFromHeaders()` for daemon header extraction.
  - **ZTNA Route** (`routes/ztna.ts`) — `POST /api/ztna/posture` (submit device signals, get score + decision, upserts `ztna_devices` DB record) + `GET /api/ztna/device/:fingerprint`. Audit + SIEM on every check.
  - **Audit Ledger upgrade** (`lib/audit-chain.ts`) — Upgraded from SHA-256 to **SHA3-256** hash chain + **HMAC-SHA512** signature per entry. Added `seq` counter, `verifyChain()` for tamper detection, `AuditLedger` class for per-tenant independent chains. Still exports `appendAuditEvent()` singleton for existing callers.
  - **SIEM module** (`lib/siem.ts`) — `shipSecurityEvent()` fans out to Splunk HEC (`SPLUNK_HEC_URL` / `SPLUNK_HEC_TOKEN`) + generic HTTP webhook (`SIEM_WEBHOOK_URL`). Always logs locally first. Silent no-op when not configured; warnings on delivery failure.
  - **DB schema** — New `ztna_devices` table (userId, certFingerprint, trustScore, posture jsonb, revoked, lastSeenAt). Migration: `migrations/20260609_ztna_schema.sql` — includes RLS policy, `pgcrypto` extension, append-only `audit_log_append_only` table + immutable trigger.
  - **Ops scripts** (`standalone/scripts/`) — 5 new: `harden-ubuntu-stig.sh` (UFW, auditd, fail2ban, sysctl hardening, SSH key-only), `generate-ca-and-mtls.sh` (4096-bit CA + 3072-bit daemon cert), `rotate-wireguard-keys.sh`, `backup-postgres.sh` (pg_dump + sha256), `openscap-scan.sh` (STIG profile).
  - **Hardened installers** (`standalone/installers/`) — `install-linux-hardened.sh` (SHA-256 config verification + nftables kill-switch), `Install-ProxHQVPN-Hardened.ps1` (Windows: hash verify + WireGuard MSI + Defender firewall kill-switch).
  - **Security tests** (`artifacts/api-server/tests/security-gap-bridge.test.ts`) — Full vitest suite: RBAC (8 cases), device trust (7 cases), AuditLedger class (3 cases), chain verification (2 cases), singleton appendAuditEvent.
