...
$BASE_URL   = "https://releases.proxhqvpn.app"
$ZIP_NAME   = "ProxhqVPN-Windows-x64.zip"
$BINARY     = "ProxhqVPN.exe"

# ── Colours / helpers ─────────────────────────────────────────────────────────
function Write-Banner {
    Write-Host ""
    Write-Host "  +----------------------------------------------+" -ForegroundColor Green
    Write-Host "  |       PROXHQVPN - INSTALLER v3.0         |" -ForegroundColor Green
    Write-Host "  +----------------------------------------------+" -ForegroundColor Green
    Write-Host ""
}
...
function Assert-Admin {
    ...
    if (-not $p.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        Write-Host ""
        Write-Host "  ProxhqVPN needs Administrator rights to install." -ForegroundColor Yellow
...
# ── Download ──────────────────────────────────────────────────────────────────
Write-Step "Downloading ProxhqVPN"
...
try {
    ...
    if (Test-Path $localZip) {
        ...
        Write-Fail "Could not download ProxhqVPN.`n  Check your internet connection.`n  Or download from: https://proxhqvpn.app/download"
    }
}
...
$tmpExtract = Join-Path $tmp "proxhqvpn-extract"
...
Write-Step "Adding ProxhqVPN to PATH"
$currentPath = [Environment]::GetEnvironmentVariable("PATH", "User")
if ($currentPath -notlike "*$Dir*") {
    [Environment]::SetEnvironmentVariable("PATH", "$currentPath;$Dir", "User")
    $env:PATH += ";$Dir"
    Write-Ok "Added to PATH — you can now type 'ProxhqVPN' from anywhere"
} else {
...
if ($Service) {
    ...
    $svcDesc = "ProxhqVPN Orchestration Platform"
    ...
    Write-Info "  Start-Service ProxhqVPN"
    Write-Info "  Stop-Service  ProxhqVPN"
    Write-Info "  Get-Service   ProxhqVPN"
}
...
try {
    ...
    $lnkPath   = Join-Path $startMenu "Programs\ProxhqVPN.lnk"
    $shell      = New-Object -ComObject WScript.Shell
    $shortcut   = $shell.CreateShortcut($lnkPath)
    $shortcut.TargetPath       = Join-Path $Dir $BINARY
    $shortcut.WorkingDirectory = $Dir
    $shortcut.Description      = "ProxhqVPN Dashboard"
    $shortcut.Save()
...
try {
    Remove-NetFirewallRule -DisplayName "ProxhqVPN" -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "ProxhqVPN" `
...
if (-not $Service) {
    Write-Info "Starting ProxhqVPN now..."
    Write-Info "To run ProxhqVPN in future: double-click the desktop shortcut"
    Write-Info "Or open Start Menu and search for 'ProxhqVPN'"
    Write-Host ""
    ...
    Write-Info "ProxhqVPN is running as a Windows Service."
