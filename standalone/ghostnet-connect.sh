#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
#  GhostNet × VPN Gate — One-Command Auto-Connect
#  Usage:
#    ./ghostnet-connect.sh                  # auto-pick best server
#    ./ghostnet-connect.sh JP               # best server in Japan
#    ./ghostnet-connect.sh 123.45.67.89     # connect to specific IP
#    ./ghostnet-connect.sh --list           # list top 10 servers
# ─────────────────────────────────────────────────────────────
set -euo pipefail
IFS=$'\n\t'
...
GHOSTNET_PORT="${GHOSTNET_PORT:-7474}"
DASHBOARD="http://localhost:${GHOSTNET_PORT}"
OVPN_FILE="/tmp/ghostnet-vpngate-$$.ovpn"
CREDS_FILE="/tmp/ghostnet-vpngate-creds-$$.txt"
ARG="${1:-}"

banner() {
  echo -e "${CYAN}"
  echo "  ╔═══════════════════════════════════════════╗"
  echo "  ║     GhostNet × VPN Gate Auto-Connect      ║"
...
list_servers() {
  ...
  if RAW=$(fetch_from_dashboard "/api/vpngate/servers?limit=10" 2>/dev/null); then
    ...
    echo -e "  ${RED}Dashboard not reachable. Start GhostNet first.${NC}"
  fi
  exit 0
}
...
if DASHBOARD_UP=$(fetch_from_dashboard "/api/vpngate/servers/best${ARG:+?country=$ARG}" 2>/dev/null) && [ -n "$DASHBOARD_UP" ]; then
  echo -e "  Source: ${CYAN}GhostNet Dashboard${NC} (cached + filtered)"
