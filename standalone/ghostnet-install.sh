#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
#  GhostNet VPN — Universal Installer
#  Works on Linux (x64) and macOS (Apple Silicon + Intel)
#
#  ONE COMMAND INSTALL:
#    curl -fsSL https://get.ghostnet.app | bash
#
#  Or with options:
#    curl -fsSL https://get.ghostnet.app | bash -s -- --port 8080 --dir ~/ghostnet
#
#  What this does:
#    1. Detects your OS and chip type
#    2. Downloads the correct GhostNet package
#    3. Extracts it to ~/GhostNet (or your chosen folder)
...
echo ""
echo -e "${GRN}  ╔══════════════════════════════════════════════╗${NC}"
echo -e "${GRN}  ║       GHOSTNET VPN — INSTALLER v3.0         ║${NC}"
echo -e "${GRN}  ╚══════════════════════════════════════════════╝${NC}"
echo ""

# ── Defaults ─────────────────────────────────────────────────────────────────
INSTALL_DIR="${HOME}/GhostNet"
PORT=7474
INSTALL_SERVICE=false
BASE_URL="https://releases.ghostnet.app"   # ← update when hosting releases
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      echo "Usage: curl -fsSL https://get.ghostnet.app | bash -s -- [OPTIONS]"
      echo ""
      echo "  --dir    <path>   Install location (default: ~/GhostNet)"
...
case "$OS" in
  Linux)
    OS_LABEL="Linux"
    case "$ARCH" in
      x86_64)  PACKAGE="GhostNet-Linux-x64" ;;
      aarch64) err "Linux ARM64 is not yet supported. Use the Universal package:\n  Download Node.js 20+ from https://nodejs.org then use GhostNet-Universal-NodeJS.zip" ;;
  ...
  Darwin)
    OS_LABEL="macOS"
    case "$ARCH" in
      arm64)  PACKAGE="GhostNet-macOS-arm64" ;;
      x86_64) PACKAGE="GhostNet-macOS-x64"   ;;
...
if [[ "$DOWNLOADER" == "curl" ]]; then
  curl -fsSL --progress-bar -o "$TMP_ZIP" "$DOWNLOAD_URL" || {
    ...
    if [[ -f "$LOCAL_ZIP" ]]; then
      ...
      err "Could not download GhostNet.\n  Check your internet connection.\n  Or download manually from: https://ghostnet.app/download"
    fi
  }
else
...
chmod +x "${INSTALL_DIR}/GhostNet"   2>/dev/null || true
...
if [[ "$OS" == "Darwin" ]]; then
  ...
  ok "Quarantine flag removed — Gatekeeper will not block GhostNet"
fi
...
if [[ "$INSTALL_SERVICE" == "true" ]]; then
  ...
  if [[ "$OS" == "Linux" ]]; then
    if ! command -v systemctl &>/dev/null; then
      ...
      USERNAME="$(whoami)"
      SERVICE_FILE="/etc/systemd/system/ghostnet.service"
      BINARY="${INSTALL_DIR}/GhostNet"
...
[Unit]
Description=GhostNet VPN Platform
...
EOF
        ...
        ok "Manage with: sudo systemctl {start|stop|status|restart} ghostnet"
      fi
    fi

  elif [[ "$OS" == "Darwin" ]]; then
    PLIST="${HOME}/Library/LaunchAgents/app.ghostnet.plist"
...
<dict>
  <key>Label</key>
  <string>app.ghostnet</string>
  <key>ProgramArguments</key>
  <array>
    <string>${INSTALL_DIR}/GhostNet</string>
  </array>
  ...
  <string>${HOME}/Library/Logs/ghostnet.log</string>
  <key>StandardErrorPath</key>
  <string>${HOME}/Library/Logs/ghostnet-error.log</string>
</dict>
</plist>
EOF
...
if [[ "$INSTALL_SERVICE" != "true" ]]; then
  ...
  if [[ "$OS" == "Linux" ]]; then
    echo "    sudo systemctl stop ghostnet"
  else
    echo "    launchctl unload ~/Library/LaunchAgents/app.ghostnet.plist"
  fi
  echo ""
fi
