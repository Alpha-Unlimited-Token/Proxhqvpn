...
# ─────────────────────────────────────────────────────────────
#  GhostNet × VPN Gate — One-Command Auto-Connect (Windows)
#  Usage:
#    .\ghostnet-connect.ps1                # auto-pick best server
#    .\ghostnet-connect.ps1 -Country JP    # best server in Japan
#    .\ghostnet-connect.ps1 -IP 1.2.3.4   # connect to specific IP
#    .\ghostnet-connect.ps1 -List          # list top 10 servers
# ─────────────────────────────────────────────────────────────
param(
  [string]$Country = "",
  [string]$IP = "",
...
$GhostNetPort = if ($env:GHOSTNET_PORT) { $env:GHOSTNET_PORT } else { "7474" }
$Dashboard = "http://localhost:${GhostNetPort}"
$TempDir = $env:TEMP
$OvpnFile = Join-Path $TempDir "ghostnet-vpngate-$PID.ovpn"
$CredsFile = Join-Path $TempDir "ghostnet-vpngate-creds-$PID.txt"

Write-Host ""
Write-Host "  ╔═══════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "  ║     GhostNet × VPN Gate Auto-Connect      ║" -ForegroundColor Cyan
...
if ($List) {
  ...
  if ($servers -and $servers.servers) {
    ...
    Write-Host "  Dashboard not reachable. Start GhostNet first." -ForegroundColor Red
  }
  exit 0
}
...
if ($best -and (-not $IP)) {
  ...
  Write-Host "  Source: GhostNet Dashboard" -ForegroundColor Cyan
