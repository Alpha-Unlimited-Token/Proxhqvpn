    ...
    items: [
      {
        ...
        steps: [
          { text: "Download the ProxhqVPN Setup .zip above, extract it, and double-click 'Launch-ProxhqVPN-Setup.vbs'. The VBS wrapper hides the console window so you get a clean GUI wizard with no black terminal appearing." },
          { text: "SmartScreen warning? Click 'More info' → 'Run anyway'. This appears for any installer downloaded outside the Microsoft Store and is safe to proceed." },
          { text: "Screen 1 — Welcome: shows all features included. Click 'Get Started'." },
          { text: "Screen 2 — License & Tunnel Mode: tick the checkbox to accept, then pick your tunnel mode. Split Tunnel (★ Recommended) keeps all your apps and games working normally. Full Tunnel encrypts everything for maximum privacy." },
          { text: "Screen 3 — Silent WireGuard Install: the wizard downloads WireGuard from the official wireguard.com, installs it with the /S silent flag, and creates your desktop and Start Menu shortcuts — no prompts or clicks needed. Phase badges track progress: ① WireGuard → ② Sign In → ③ Activate." },
          { text: "Screen 4 — Sign In (inside the wizard): a built-in sign-in panel opens ProxhqVPN directly inside the installer window. Sign in with your account. The wizard detects successful sign-in automatically." },
          { text: "Screen 5 — Auto-Activate: the moment sign-in is detected the wizard calls the ProxhqVPN API, generates your WireGuard config, applies your tunnel mode setting (Split = 10.8.0.0/24 / Full = 0.0.0.0/0), and runs wireguard.exe /installtunnel — the VPN is live with zero manual steps." },
          { text: "Screen 6 — Done: status grid confirms WireGuard, tunnel mode, active connection, and shortcuts. Click 'Open ProxhqVPN' to go to your dashboard." },
        ],
        note: "Everything happens inside the wizard — WireGuard installs silently, sign-in is embedded, and the VPN activates automatically. No browser windows, no manual config import, no extra downloads required.",
      },
      {
