...
export default function Dashboard() {
  ...
  const { data: nodeStats } = useGetNodeStats();
  const { data: beaconStats } = useGetBeaconStats();
  const { data: fwStatus } = useGetFirewallStatus();
  
  const { data: recentAlerts } = useListBeaconAlerts({ status: 'active' });
