...
export default function OmniStrike() {
  ...
  // In chain mode, include partial findings from the currently-running phase so results stream live
  const allFindings: Finding[] = orchMode === "chain"
    ? [
        ...chainScans.flatMap(s => s.findings ?? []),
        ...(chainRunning && activeScan?.status === "running" ? (activeScan.findings ?? []) : []),
      ]
    : (activeScan?.findings ?? []);
  ...
  const exportReport = useCallback(() => {
    ...
    const bypassedF = allF.filter(f => f.bypassed);

    const SEV_ORDER: Record<string, number> = { critical: 0, high: 1, medium: 2, low: 3 };
    const sorted = [...allF].sort((a, b) => (SEV_ORDER[a.severity] ?? 9) - (SEV_ORDER[b.severity] ?? 9));

    const findingBlock = (f: typeof allF[0], idx: number) => [
      ``,
      `### ${idx + 1}. [${f.severity.toUpperCase()}] ${f.category} — ${f.technique}`,
      `| Field | Value |`,
      `|-------|-------|`,
      `| URL | \`${f.url}\` |`,
      `| Parameter | \`${f.param}\` |`,
      ...
      `| Status | ${f.bypassed ? "✅ CONFIRMED EXPLOITABLE" : "ℹ️ Informational"} |`,
      ``,
      `**Payload:**`,
      `\`\`\``,
      f.payload,
    ...
    const lines = [
      ...
      `---`,
      `## Summary`,
      `| Severity | Total | Confirmed |`,
      `|----------|-------|-----------|`,
      `| Critical | ${allF.filter(f=>f.severity==="critical").length} | ${allF.filter(f=>f.severity==="critical"&&f.bypassed).length} |`,
      `| High     | ${allF.filter(f=>f.severity==="high").length} | ${allF.filter(f=>f.severity==="high"&&f.bypassed).length} |`,
      `| Medium   | ${allF.filter(f=>f.severity==="medium").length} | ${allF.filter(f=>f.severity==="medium"&&f.bypassed).length} |`,
      `| Low      | ${allF.filter(f=>f.severity==="low").length} | ${allF.filter(f=>f.severity==="low"&&f.bypassed).length} |`,
      `| **Total**| **${allF.length}** | **${bypassedF.length}** |`,
      ``,
      `---`,
      `## All Findings (${allF.length} total, sorted by severity)`,
      ...sorted.map((f, i) => findingBlock(f, i + 1)),
      ``,
      `---`,
    ];

    if (bypassedF.length > 0) {
      lines.push(
        `## Confirmed Exploitable Findings (${bypassedF.length})`,
        ...bypassedF.map((f, i) => [
          ``,
          ...
          ...(f.canExec ? [`- ⚠️ **RCE (Remote Code Execution) confirmed**`] : []),
          ...(f.canRead ? [`- ⚠️ **File read / LFI confirmed**`] : []),
        ].join("\n")),
        ``,
        `---`,
      );
      ...
      lines.push(
        ...
        ``,
        `> No confirmed exploitable vulnerabilities in this scan. All ${allF.length} findings are informational or require manual verification.`,
        ``,
        `---`,
      );
    }
    ...
    a.click();
    toast({ title: `Report downloaded — ${allF.length} findings` });
