      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {[
            ...
            { claim: "Full security research toolkit for professionals", detail: "Command Center Pro includes Ghost Chain, OSINT Recon, Canary Tokens, SIEM, Dependency Scanner, WebSocket Tester, SAST Scanner, QuantumAudit, and more — all routed through the VPN for operational security." },
            { claim: "Honeypot + threat detection built in", detail: "SilkWeb honeypot, DNS Sinkhole, Firewall Manager, Canary Tokens, and Ghost Trap counter-intelligence platform form a complete defensive layer — not just a privacy tool." },
            { claim: "Automated node lifecycle management", detail: "The Node Lifecycle Engine automatically rotates stale VPN nodes every 15 minutes, converting old node IPs into honeypot traps that catch attackers automatically. Every node rotation is a security event — not just maintenance." },
            { claim: "Attacker banner grabbing & service fingerprinting", detail: "The Attacker Intelligence Console includes a Banner Grab tab that reads TCP service banners and HTTP headers from trapped attacker IPs — identifying the exact software, version, and OS the attacker is running without alerting them to a full port scan." },
            { claim: "Ghost Trap — real-time counter-intelligence against attackers", detail: "Ghost Trap detects, fingerprints, and wastes attackers' time with a 7-stage deception pipeline. It includes a Manual IP Investigator: paste any suspicious IP you spotted in netstat or your server logs and instantly get the geo, ISP, port status, reverse DNS, and abuse contact — no configuration needed." },
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          {[
            ...
            {
              num: "8", tip: "Lead with Enterprise Security Features for Corporate & IT Audiences",
              detail: "ProxhqVPN's 2026 security upgrades are a strong pitch for IT security managers, DevSecOps teams, and enterprise buyers. Key talking points: (1) RBAC with 6 roles — owner, security_admin, network_admin, auditor, support, user — so teams can have least-privilege access without sharing admin credentials. (2) ZTNA device posture scoring — devices score 0–100 on 8 signals before receiving a WireGuard config; rooted/unpatched devices are blocked automatically. (3) SHA3-256 + HMAC-SHA512 tamper-evident audit chain — every admin action is cryptographically chained so deletions and modifications are detectable. (4) SIEM fanout to Splunk HEC + generic webhooks — compatible with Elastic, Datadog, and PagerDuty out of the box. (5) Node Lifecycle Engine automatically rotates stale nodes and converts old IPs to honeypot traps. Frame it as: 'Enterprise-grade Zero Trust VPN — without the enterprise price tag.'",
            },
            {
              num: "9", tip: "Showcase the Attacker Intelligence Console for Red Team & SOC Audiences",
              detail: "The Attacker Intel Panel is a strong differentiator for red teamers and SOC analysts. New in 2026: the Banner Grab tab reads TCP service banners and HTTP headers from trapped attacker IPs — identifying the exact software, version, and OS without triggering a full port scan. Combined with Port Scan, OS Shell, File Manager, and Worm Callback intelligence, the console turns every honeypot hit into a full counter-intelligence operation. Talking point: 'When an attacker hits your honeypot, you immediately know what software they're running, what ports they have open, and can extract files from their system — all from inside the dashboard.' This angle converts strongly for cybersecurity bootcamp graduates, SOC analysts, and red team consultants.",
            },
          ].map(({ num, tip, detail }) => (
    ...
    content: (
      <div className="space-y-3">
        ...
        {[
          ...
          { q: "How is ProxhqVPN different from NordVPN / ExpressVPN?", a: "The biggest difference is the Command Center Pro tier — it includes a full security research platform (Ghost Chain, OSINT Recon, SIEM, QuantumAudit, SilkWeb honeypot, Canary Tokens, DNS Sinkhole, and more) that no consumer VPN provider offers. ProxhqVPN is built for developers and security professionals, not just streaming — features in a completely different category." },
          ...
          { q: "Is the Command Center legal?", a: "All tools in the Command Center are standard industry security tools used by professional penetration testers daily. They are legal to own and use for authorized testing. They are comparable to industry-standard defensive security tools and all include prominent legal disclaimers." },
...
<div class="slide"><h2>What is ProxhqVPN?</h2><p>ProxhqVPN is not just a VPN — it's a complete privacy and security operations platform for individuals, developers, and security professionals.</p><br><div class="grid"><div class="card"><h3>🔒 Privacy Layer</h3><p>60-node WireGuard mesh, kill switch, DNS Sinkhole, Tor integration, no-log policy, warrant canary</p></div><div class="card"><h3>🛡️ Defense Layer</h3><p>SilkWeb honeypot, Canary Tokens, Firewall Manager, Ghost Trace behavioral analysis, Beacon Monitor</p></div><div class="card"><h3>🔍 Security Research</h3><p>OSINT Recon, Ghost Chain, QuantumAudit, HTTP Probe, CVE Lookup, JWT Analyzer, API Tester</p></div><div class="card"><h3>🔗 Infrastructure Layer</h3><p>QuantumAudit blockchain scanner, IaC Scanner, API Security Tester, WAF Analyzer, LLM Probe</p></div></div></div>
