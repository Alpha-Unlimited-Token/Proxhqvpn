...
function TerminalLine({ line }: { line: string }) {
  ...
  const color = isCancelled ? "text-orange-400" : isError ? "text-red-400" : isWarning ? "text-yellow-400" : isSuccess ? "text-primary" : isInfo ? "text-white/60" : "text-white/40";
...
function PortLine({ line }: { line: string }) {
  ...
  const color = isOpen ? "text-primary font-semibold" : isInfo ? "text-blue-400" : "text-white/45";
  return <div className={`text-xs font-mono leading-relaxed ${color}`}>{line}</div>;
}
            ...
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`flex items-center gap-2 flex-1 ${i < STEPS.length - 1 ? "min-w-0" : ""}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border transition-all ${
                  ...
                  {done ? <CheckCircle className="w-3.5 h-3.5 text-primary" /> : <Icon className={`w-3.5 h-3.5 ${active ? "text-primary" : "text-white/25"}`} />}
                </div>
                <span className={`text-xs font-medium hidden sm:block ${active ? "text-white/80" : done ? "text-primary/60" : "text-white/25"}`}>{s}</span>
              </div>
      ...
      {step === 0 && (
        <div className="bg-[#0d1610] border border-white/[0.07] rounded-2xl p-6 space-y-5">
          ...
          <div className="text-xs text-white/30 leading-relaxed">
            ...
            <span className="font-mono text-white/40"> ?id=1</span>. For full site scanning, just enter the homepage.
          </div>
        </div>
      )}
        ...
        <div className="bg-[#0d1610] border border-white/[0.07] rounded-2xl p-6 space-y-6">
              ...
              {[1, 2, 3].map(r => (
                <button key={r} onClick={() => setConfig(c => ({ ...c, risk: r }))}
                  className={`py-2.5 rounded-xl border text-xs font-medium transition-all ${
                    config.risk === r ? `${RISK_LABELS[r].color} bg-white/5 border-white/15` : "text-white/35 border-white/[0.06] hover:border-white/15"
                  }`}>
                  {RISK_LABELS[r].label}
                </button>
              ))}
          ...
          <div className="space-y-3 border border-primary/15 bg-primary/[0.03] rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-sm font-semibold text-white/80">Page Crawl &amp; Site Coverage</div>
            </div>
            ...
            {!config.fullCrawl && (
              <div className="space-y-2 pt-1">
                <div className="flex items-center justify-between">
                  <div className="text-xs text-white/50 font-medium">Manual crawl depth</div>
          ...
          <div className="border border-blue-500/15 bg-blue-500/[0.03] rounded-2xl p-4 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-sm font-semibold text-white/80">Port Scan</div>
          ...
          {/* Advanced */}
          <button onClick={() => setShowAdvanced(v => !v)} className="flex items-center gap-2 text-xs text-white/30 hover:text-white/50 transition-colors">
          ...
          <div className="flex gap-1 p-1 bg-white/[0.03] border border-white/[0.07] rounded-xl">
            <button onClick={() => setActiveTab("sqli")}
              className={`flex-1 py-2 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 transition-all ${
                activeTab === "sqli" ? "bg-primary/15 text-primary" : "text-white/40 hover:text-white/60"
              }`}>
            ...
            {config.portScan && (
              <button onClick={() => setActiveTab("ports")}
                className={`flex-1 py-2 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 transition-all ${
                  activeTab === "ports" ? "bg-blue-400/15 text-blue-400" : "text-white/40 hover:text-white/60"
                }`}>
          ...
          {activeTab === "sqli" && (
            <div className="bg-[#060b07] border border-white/[0.07] rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-white/[0.05] bg-white/[0.02]">
                ...
                {scanDone && <span className="ml-auto text-xs text-white/20 font-mono">done · {sqlLines.length} lines</span>}
              </div>
            ...
            {allDone && (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-3 rounded-2xl border border-white/[0.08] text-sm text-white/50 hover:text-white/80 hover:bg-white/[0.05] transition-all"
                >
                ...
                <button
                  onClick={resetScan}
                  className="flex items-center gap-2 px-5 py-3 rounded-2xl border border-white/[0.08] text-sm text-white/50 hover:text-white/80 hover:bg-white/[0.05] transition-all ml-auto"
                >
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-40 ${
              step === 2 ? "bg-primary text-black hover:brightness-110 shadow-[0_0_30px_rgba(0,255,136,0.2)]" : "bg-white/[0.07] text-white/80 hover:bg-white/[0.1]"
            }`}>
            {startScan.isPending ? (
