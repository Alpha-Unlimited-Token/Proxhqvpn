...
function resolveServerUrl() {
  ...
  if (process.env.PROXHQ_SERVER_URL) return process.env.PROXHQ_SERVER_URL;

  // 2. Read from Replit deployed domain env
  if (process.env.REPLIT_DEV_DOMAIN) {
    return `https://${process.env.REPLIT_DEV_DOMAIN}`;
  }

  // 3. Try to read from the artifact.toml if present
  const tomlPath = path.resolve(__dirname, "../../../artifacts/ghost-vpn/.replit-artifact/artifact.toml");
  if (fs.existsSync(tomlPath)) {
    const toml = fs.readFileSync(tomlPath, "utf8");
    const m = toml.match(/previewPath\s*=\s*"([^"]+)"/);
    if (m) console.log(`[prepare] Found artifact preview path: ${m[1]}`);
  }

  // 4. Prompt the user in CI
  console.warn(
    "\n⚠  PROXHQ_SERVER_URL not set.\n" +
    "   Set it before building:\n" +
    "   export PROXHQ_SERVER_URL=https://your-domain.replit.app\n" +
    "   Defaulting to placeholder — remember to update before distributing.\n"
  );
  return "https://your-proxhqvpn-domain.replit.app";
}

// ── Write config.js with live URL ─────────────────────────────────────────
function writeConfig(url) {
  const content = `// AUTO-GENERATED by scripts/prepare-build.js — do not edit manually
// Rebuild to update the server URL.
const PROXHQ_SERVER_URL = ${JSON.stringify(url)};
module.exports = { PROXHQ_SERVER_URL };
`;
  fs.writeFileSync(CONFIG_JS, content);
  console.log(`[prepare] Server URL set to: ${url}`);
}
