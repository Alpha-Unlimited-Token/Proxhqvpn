...
import {
  createAdminActivityTimeline,
  listAdminActivityTimeline,
} from "../services/adminActivityTimelineService";
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await createAdminActivityTimeline({
      ...body,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ items: await listAdminActivityTimeline(query) });
  }),
);
