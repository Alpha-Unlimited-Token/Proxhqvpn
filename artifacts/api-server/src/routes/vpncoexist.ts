...
const VPN_PROFILES = [
  ...
  },
  {
    ...
    killSwitchIface: "tun0",
    notes: "Surfshark NoBorders mode uses obfuscation. MultiHop option pairs well with PROXHQ overlay.",
    coexistMethod: "routing-table",
  },
  {
    ...
    serverCidrs: [],
    killSwitchIface: "wg0",
    notes: "Generic WireGuard interface. PROXHQ runs on a separate interface (tun0 or proxhq0).",
    coexistMethod: "fwmark",
  },
  {
    id: "openvpn-generic",
    ...
    serverCidrs: [],
    killSwitchIface: "tun0",
    notes: "Generic OpenVPN adapter. Use routing tables 200/201 to separate PROXHQ and OpenVPN.",
    coexistMethod: "routing-table",
  },
];
...
# PROXHQ VPN Coexistence — fwmark Policy Routing
# Mode: fwmark (traffic marked ${proxhqFwmark} → PROXHQ, rest → ${detectedVpnIface})
# Generated by PROXHQ v3.0 — run as root

set -e

PROXHQ_IFACE="${ghostnetIface}"
NATIVE_VPN_IFACE="${detectedVpnIface}"
FWMARK=${proxhqFwmark}
TABLE=${tableId}

echo "[+] Setting up PROXHQ coexistence routing (fwmark mode)..."

# 1. Create routing table for PROXHQ traffic
ip route flush table $TABLE 2>/dev/null || true
ip route add default dev $PROXHQ_IFACE table $TABLE

# 2. Add fwmark rule: marked packets use PROXHQ table
ip rule del fwmark $FWMARK table $TABLE 2>/dev/null || true
ip rule add fwmark $FWMARK table $TABLE priority 100

# 3. Native VPN exception routes (bypass PROXHQ for native VPN traffic)
${exceptions_str || "# No additional exceptions added"}

# 4. iptables: mark PROXHQ-destined traffic
...
# Mark traffic from PROXHQ TUN (10.99.0.0/24) with fwmark
...
echo "[+] PROXHQ coexistence table:"
ip route show table $TABLE

echo "[+] Done. PROXHQ runs alongside $NATIVE_VPN_IFACE."
echo "    • PROXHQ traffic: tun/proxhq → fwmark $FWMARK → table $TABLE"
...
`;
  ...
  if (mode === "double-hop") {
    return `#!/usr/bin/env bash
# PROXHQ VPN Coexistence — Double-Hop Mode
# Traffic path: Your Device → PROXHQ Mesh (${ghostnetIface}) → ${detectedVpnIface} → Internet
...
# Step 1: PROXHQ runs first (inner tunnel)
# Its packets exit via the native VPN as outer tunnel
# This is achieved by ensuring PROXHQ daemon traffic
# uses the native VPN's routing table

# MTU adjustment — critical for double-hop
# Ethernet (1500) → Native VPN (~1420) → PROXHQ (~1380)
ip link set $PROXHQ_IFACE mtu 1380 2>/dev/null || true
echo "[+] MTU set to 1380 on $PROXHQ_IFACE (double-hop optimized)"

# Step 2: Route PROXHQ control traffic through native VPN
# The PROXHQ server endpoint must go through the native VPN
GHOSTNET_SERVER_IP="\${GHOSTNET_SERVER_IP:-10.99.0.1}"
ip route add $GHOSTNET_SERVER_IP/32 dev $NATIVE_VPN_IFACE 2>/dev/null || true
echo "[+] PROXHQ server endpoint routed through native VPN"

# Step 3: All other traffic through PROXHQ (which exits via native VPN)
ip route replace default dev $PROXHQ_IFACE

echo "[+] Double-hop active:"
echo "    Device → PROXHQ ($PROXHQ_IFACE) → Native VPN ($NATIVE_VPN_IFACE) → Internet"
echo ""
echo "    Privacy: Your ISP sees only native VPN traffic."
echo "    Native VPN provider sees only encrypted PROXHQ packets."
...
`;
  ...
  // namespace mode
  return `#!/usr/bin/env bash
# PROXHQ VPN Coexistence — Network Namespace Isolation
# PROXHQ runs in its own namespace; native VPN owns the main namespace
# Generated by PROXHQ v3.0 — run as root
...
echo "[+] Creating network namespace '$NS' for PROXHQ..."
...
echo "    Run PROXHQ inside namespace:"
echo "    sudo ip netns exec $NS ./ghostd.py --mode server"
echo ""
echo "    Native VPN continues in main namespace (unchanged)."
echo "    PROXHQ and native VPN are fully isolated — no conflicts."
`;
}

function generateWindowsCoexistScript(detectedVpnIface: string, mode: string): string {
  return `@echo off
:: PROXHQ VPN Coexistence — Windows Route Table
:: Mode: ${mode}
:: Generated by PROXHQ v3.0 — Run as Administrator

echo [+] PROXHQ VPN Coexistence Setup (Windows)
echo     Native VPN interface: ${detectedVpnIface || "Auto-detected"}
echo.

:: Set higher metric on PROXHQ adapter to avoid conflicts
:: Lower number = preferred. PROXHQ should NOT override the native VPN
:: unless explicitly configured.

:: Find PROXHQ tunnel adapter (WinTun / PROXHQ0)
netsh interface ipv4 show interfaces | findstr /i "ghostnet wintun tun"

:: Set interface metrics
:: PROXHQ: metric 50 (handles specific traffic)
:: Native VPN: metric 5 (default, handles everything else)
netsh interface ipv4 set interface "PROXHQ" metric=50 2>nul
netsh interface ipv4 set interface "${detectedVpnIface || "NordLynx"}" metric=5 2>nul

:: Add specific routes through PROXHQ tunnel
:: Example: route all 10.99.0.0/24 (PROXHQ mesh) through tun
...
${mode === "double-hop" ? 'netsh interface ipv4 set subinterface "PROXHQ" mtu=1380 store=persistent' : ":: (MTU adjustment not needed in this mode)"}

echo.
echo [+] Routes configured. Verify with: route print
echo [+] PROXHQ and ${detectedVpnIface || "Native VPN"} are now co-active.
pause
`;
}
...
function generateMacOSCoexistScript(detectedVpnIface: string, mode: string): string {
  return `#!/usr/bin/env bash
# PROXHQ VPN Coexistence — macOS pf + route
# Mode: ${mode}
# Generated by PROXHQ v3.0 — run as root (sudo)

set -e

PROXHQ_IFACE="${mode === "namespace" ? "utun10" : "utun5"}"
NATIVE_VPN_IFACE="${detectedVpnIface || "utun0"}"

echo "[+] PROXHQ macOS coexistence setup..."

# macOS uses utun interfaces for VPN tunnels
# Check active interfaces
ifconfig | grep -E "^utun" | awk '{print $1}'

# Set routing for PROXHQ mesh (10.99.0.0/24) to PROXHQ interface
route -n add -net 10.99.0.0/24 -interface $PROXHQ_IFACE 2>/dev/null || true
echo "[+] PROXHQ mesh routed via $PROXHQ_IFACE"

# Set MTU for PROXHQ tunnel (avoid fragmentation)
${mode === "double-hop" ? "ifconfig $PROXHQ_IFACE mtu 1380" : "ifconfig $PROXHQ_IFACE mtu 1420"}
echo "[+] MTU configured"
...
# PROXHQ coexistence pf anchor
# Block PROXHQ-sourced traffic from leaking to native VPN iface
pass out on $PROXHQ_IFACE all
pass in on $PROXHQ_IFACE all
# Do not redirect PROXHQ control port (7475) through native VPN
...
echo ""
echo "[+] PROXHQ ($PROXHQ_IFACE) and native VPN ($NATIVE_VPN_IFACE) co-active."
...
# PROXHQ coexistence DISABLE script
...
:: Undo PROXHQ coexistence (Windows)
route delete 10.99.0.0
netsh interface ipv4 set interface "PROXHQ" metric=automatic 2>nul
echo [+] PROXHQ coexistence rules removed.
`;
  }
  ...
  res.json({
    ...
    notes: [
      ...
      `MTU for PROXHQ tunnel set to ${mtu}${mode === "double-hop" ? " (double-hop reduces MTU by 40 bytes per hop)." : "."}`,
      profile.notes,
    ],
  });
});
...
router.post("/mtu-optimize", (req, res) => {
  ...
  res.json({
    ...
    breakdown: Array.from({ length: hopCount }, (_, i) => ({
      hop: i + 1,
      label: i === 0 ? "Native VPN" : `PROXHQ hop ${i}`,
      mtu: baseMtu - perHopOverhead * (i + 1),
    })),
    commands: {
      linux: `ip link set ghostnet0 mtu ${recommended}`,
      macos: `ifconfig utun5 mtu ${recommended}`,
      windows: `netsh interface ipv4 set subinterface "PROXHQ" mtu=${recommended} store=persistent`,
    },
