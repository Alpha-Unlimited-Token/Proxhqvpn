...
router.post("/headers-inspect", async (req, res) => {
  ...
  try {
    const resp = await fetch(url, {
      ...
      headers: { "User-Agent": "GhostNet-SecurityAudit/3.0" },
    });
...
router.get("/self-audit", (_req, res) => {
  const findings: { category: string; severity: "critical"|"high"|"medium"|"low"|"info"; title: string; description: string; remediation: string }[] = [
    ...
    },
    {
      ...
      remediation: "Enable API_KEY auth. Only expose GhostNet on a private network. The terminal is designed for trusted admin use only.",
    },
    {
    ...
    },
    {
      ...
      description: "The standalone server listens on plain HTTP by default. Traffic between client browser and GhostNet server is unencrypted.",
